...
public class StudentNetworkSimulator2 extends NetworkSimulator
{
    ...
    public StudentNetworkSimulator2(int numMessages,
                                   double loss,
                                   double corrupt,
                                   double avgDelay,
                                   int trace,
    ...
    protected void aInput(Packet packet)
    {
    	if (astate == 0) {}
    	if (astate == 1) {
    		int checksum = calculateChecksum(packet.getPayload(), packet.getSeqnum(),
    				packet.getAcknum());
    ...
    protected void bInput(Packet packet)
    {
		...
		if(packet.getChecksum() != checksum) {
			...
			if(((packet.getSeqnum() == 0) && (bstate == 0)) || 
				...
				bstored_pkt = new Packet(packet); // Eventuellement modifier le packet
				this.toLayer3(B, bstored_pkt);
				System.out.println("B: sending ACK" + "Something");
				bstate = (bstate + 1)%2;
				bOnceThru = 1;
			} else {
				if ((bstate == 1) || (bOnceThru == 1)) {
					this.toLayer3(B, packet); // Eventuellement modifier le packet
					System.out.println("B: sending ACK" + "Something");
				}
			}
		}
